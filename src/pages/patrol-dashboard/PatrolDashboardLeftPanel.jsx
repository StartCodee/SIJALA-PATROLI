...
function LeftMiniMetric({ title, value }) {
  return (
    <div className="min-h-[5.5rem] border-r border-t border-b border-white/12 bg-[#183848]/65 px-3 py-2 last:border-r-0">
      <p className="min-h-8 text-[9px] font-bold uppercase leading-tight text-slate-300/80">{title}</p>
      <p className="mt-3 text-sm font-black leading-tight text-white">{value}</p>
    </div>
  );
}

function PatrolStatusBadge({ label, active }) {
  return (
    <div className={`flex w-full flex-col rounded-[7px] px-2 py-1.5 text-center ${active ? "bg-[#1ea51e]" : "bg-[#d11f1f]"}`}>
  ...
  return (
    ...
    >
      <div className="min-w-0">
        ...
        <p className="mt-1 text-[10px] font-medium leading-tight text-slate-400">{item.dateText}</p>
      </div>

      <div className="mt-2.5 grid grid-cols-2 gap-1.5">
        <PatrolStatusBadge label="Ada Temuan" active={item.hasFinding} />
        <PatrolStatusBadge label="Ada Pelanggaran" active={item.hasViolation} />
      </div>
    </button>
  );
}

export default function PatrolDashboardLeftPanel({
  kkpOptions,
  selectedKkp,
  selectedPos,
  availablePosOptions,
    ...
    <aside className="flex min-h-0 flex-col space-y-0 overflow-hidden rounded-xl border border-white/15 bg-[#163848]/75 p-0 text-white xl:col-span-3">
      <div className="flex items-center justify-between border-b border-white/12 bg-[#1a4452]/75 px-3 py-3">
        <div className="flex items-center gap-2">
          <div className="grid h-8 w-8 place-items-center rounded-full bg-[#2754ff] text-[11px] font-black text-white">F</div>
      ...
      <div className="grid grid-cols-3">
        <LeftBigMetric title="Total Laporan Masuk" value={totalReports} valueClassName="text-[#ff3b20]" />
        <LeftBigMetric title="Total Laporan Tertunda" value={pendingReports} valueClassName="text-[#ff9b24]" />
        <LeftBigMetric title="Total Laporan yang telah disetujui" value={approvedReports} valueClassName="text-[#37d437]" />
      </div>
      ...
      <div className="flex min-h-0 flex-1 flex-col px-4 py-4">
        <div className="flex items-center justify-between gap-3">
          ...
          {selectedFindingId ? (
            <button
              ...
              className="inline-flex items-center gap-1 rounded-md border border-white/20 bg-white/10 px-2 py-1 text-[10px] text-white"
            >
              Reset
            </button>
          ) : null}
        ...
        <div className="custom-scrollbar dashboard-scrollbar mt-4 min-h-0 flex-1 space-y-2 overflow-y-auto pr-1">
          {leftPanelFeed.length === 0 ? (
            ...
            leftPanelFeed.map((item) => (
              <ReportFeedCard
                key={item.id}
                item={item}
                active={selectedFindingId === item.id}
