...
function PatrolDetailTable({ title, rows, footer }) {
  return (
    <div className="overflow-hidden rounded-xl border border-slate-300 bg-white text-slate-900 shadow-sm">
      ...
      {footer ? (
        <div className="border-t border-slate-300 px-3 py-2 text-right text-[11px] font-semibold">{footer}</div>
      ) : null}
    </div>
  );
}
...
function PatrolReportDetailPanel({
  report,
  draft,
  relatedPhotos,
  onDraftChange,
  ...
  const dateTimeText = formatReportDate(
    report.dateValue || "",
    String(report.summary?.waktuTemuan || "").match(/(\d{1,2}:\d{2})/)?.[1] || "08:00"
  );
  ...
  return (
    <div className="custom-scrollbar dashboard-scrollbar min-h-0 flex-1 space-y-3 overflow-y-auto pr-1">
      <div className="overflow-hidden rounded-xl border border-white/15 bg-[#1a4452]/80">
        <div className={canValidate ? "grid grid-cols-[1fr_5.25rem_5.25rem]" : "block"}>
          <div className={`${canValidate ? "border-r border-white/10" : ""} bg-[#215067] px-3 py-3`}>
            <p className="text-[11px] font-bold leading-tight text-white">
              Area {report.areaLabel} | Pos {report.posLabel}
            </p>
            <p className="mt-1 text-[10px] leading-tight text-cyan-100/90">
              Koordinator: {report.koordinatorLabel} | Speedboat: {report.speedboatLabel}
            </p>
            <p className="mt-1 text-[10px] font-semibold text-cyan-100/85">{dateTimeText}</p>
          </div>
          {canValidate ? (
            <>
              <button
      ...
      <PatrolDetailTable
        ...
        rows={[
          {
            label: "Koordinator",
            value: <input value={draft.attendanceKoordinator} onChange={(event) => onDraftChange("attendanceKoordinator", event.target.value)} className="w-full rounded border border-slate-300 px-2 py-1" />,
          },
          {
            label: "Pencatat Posisi",
            value: <input value={draft.attendancePencatatPosisi} onChange={(event) => onDraftChange("attendancePencatatPosisi", event.target.value)} className="w-full rounded border border-slate-300 px-2 py-1" />,
          },
          {
            label: "Pencatat Pemanfaatan",
            value: <input value={draft.attendancePencatatPemanfaatan} onChange={(event) => onDraftChange("attendancePencatatPemanfaatan", event.target.value)} className="w-full rounded border border-slate-300 px-2 py-1" />,
          },
        ]}
      />
        ...
        rows={[
          {
            ...
            value: <input value={draft.fuelLiters} onChange={(event) => onDraftChange("fuelLiters", event.target.value)} className="w-full rounded border border-slate-300 px-2 py-1" />,
          },
          {
            label: "Keterangan / Catatan",
            value: <input value={draft.fuelNotes} onChange={(event) => onDraftChange("fuelNotes", event.target.value)} className="w-full rounded border border-slate-300 px-2 py-1" />,
          },
          {
            label: "Tempat Bermalam",
            value: <input value={draft.restStop} onChange={(event) => onDraftChange("restStop", event.target.value)} className="w-full rounded border border-slate-300 px-2 py-1" />,
          },
          {
            label: "Deskripsi Wilayah Tujuan patroli",
            value: <textarea value={draft.routeDescription} onChange={(event) => onDraftChange("routeDescription", event.target.value)} className="min-h-[5rem] w-full rounded border border-slate-300 px-2 py-1" />,
          },
          {
            ...
            value: (
              <div className="space-y-2">
                ...
                <button
                  type="button"
                  className="inline-flex rounded-md border border-slate-300 px-2 py-1 text-[10px] font-semibold text-slate-700 hover:bg-slate-100"
                >
                  Klik ke arah peta sketsa
                </button>
              </div>
            ),
          },
      ...
      <PatrolDetailTable
        ...
        rows={[
          {
            label: "Ada / Tidak Temuan",
            value: (
              <select value={draft.hasFinding} onChange={(event) => onDraftChange("hasFinding", event.target.value)} className="w-full rounded border border-slate-300 px-2 py-1">
          ...
          },
          {
            ...
            value: (
              <div className="flex items-center justify-between gap-2">
                <input value={draft.coordinateText} onChange={(event) => onDraftChange("coordinateText", event.target.value)} className="w-full rounded border border-slate-300 px-2 py-1" />
                <button
                  type="button"
          ...
          {
            label: "Nomor GPS",
            value: <input value={draft.gpsId} onChange={(event) => onDraftChange("gpsId", event.target.value)} className="w-full rounded border border-slate-300 px-2 py-1" />,
          },
          {
            label: "Nama Lokasi Temuan",
            value: <input value={draft.findingLocationName} onChange={(event) => onDraftChange("findingLocationName", event.target.value)} className="w-full rounded border border-slate-300 px-2 py-1" />,
          },
          {
            label: "Foto Temuan",
            fullWidth: true,
            value: <ReportPhotoCarousel photos={relatedPhotos} />,
          },
        ]}
      />
    </div>
...
export default function PatrolDashboardRightPanel({
  selectedFinding,
  activeReportDraft,
  relatedDetailPhotos,
  onDraftChange,
  ...
  return (
    <aside className="flex min-h-0 flex-col overflow-hidden rounded-xl border border-white/15 bg-[#163848]/75 p-3 xl:col-span-3">
      {selectedFinding ? (
        <PatrolReportDetailPanel
          report={selectedFinding}
          draft={activeReportDraft}
          relatedPhotos={relatedDetailPhotos}
          onDraftChange={onDraftChange}
          onFocusLocation={onFocusLocation}
          canValidate={selectedFinding.statusKey === "incoming"}
          onReject={onReject}
          onApprove={onApprove}
          onDelete={onDelete}
          showDeleteAction={showDeleteAction}
          onToggleDeleteAction={onToggleDeleteAction}
        />
      ) : (
