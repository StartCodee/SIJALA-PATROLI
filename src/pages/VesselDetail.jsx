...
import { getConnectionStatus, formatRelativeTime, filterTrackPointsByTime, } from '@/data/mockData';
const VesselDetail = () => {
    const { id } = useParams();
    const navigate = useNavigate();
    const { getVesselById, trackPoints, getPatrolsByVesselId, getIncidentsByVesselId, patrols } = useData();
    const trackTimeRange = 'today';
    ...
    if (!vessel) {
        return (<MainLayout title="Kapal Tidak Ditemukan">
    ...
    return (<MainLayout title={vessel.name} subtitle={`Tanda Panggil: ${vessel.callSign}`}>
      {/* Back button */}
      <Button variant="ghost" size="sm" className="mb-4 -ml-2" onClick={() => navigate('/vessels')}>
      ...
      <Tabs defaultValue="overview" className="space-y-6">
        <TabsList>
          ...
          <TabsTrigger value="track">Lintasan</TabsTrigger>
          <TabsTrigger value="logs">Catatan</TabsTrigger>
        </TabsList>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card className="shadow-card">
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <p className="text-sm text-muted-foreground">Tanda Panggil</p>
                ...
                {currentPatrol && (<div className="pt-4 border-t border-border">
                    <p className="text-sm text-muted-foreground mb-2">Patroli Saat Ini</p>
                    <div className="p-3 rounded-lg bg-primary/5 border border-primary/20 cursor-pointer hover:bg-primary/10 transition-colors" onClick={() => navigate(`/patrols/${currentPatrol.id}`)}>
            ...
            <Card className="shadow-card">
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-sm text-muted-foreground">Lintang</p>
                    <p className="font-medium font-mono">{vessel.lastPosition.lat.toFixed(6)}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Bujur</p>
                  ...
                  <div>
                    <p className="text-sm text-muted-foreground">Pembaruan</p>
        ...
        <TabsContent value="track" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2">
              <MapCard title="Rute Perjalanan" selectedVessel={{
            name: vessel.name,
            ...
            <Card className="shadow-card">
              <CardHeader className="pb-3">
                <CardTitle className="text-base font-semibold">Titik Lintasan</CardTitle>
              </CardHeader>
              <CardContent>
                ...
                <ScrollArea className="h-[250px]">
                  <div className="space-y-2">
                    {vesselTrackPoints.slice(-10).reverse().map((point, idx) => (<div key={idx} className="text-xs p-2 rounded bg-muted/50 font-mono">
        ...
        <TabsContent value="logs" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card className="shadow-card">
              ...
              <CardContent>
                {vesselPatrols.length === 0 ? (<p className="text-sm text-muted-foreground text-center py-4">
                    Belum ada riwayat patroli
                  </p>) : (<div className="space-y-3">
                    {vesselPatrols.map((patrol) => (<div key={patrol.id} className="p-3 rounded-lg border border-border hover:border-primary/30 cursor-pointer transition-colors" onClick={() => navigate(`/patrols/${patrol.id}`)}>
                        <div className="flex items-center justify-between">
            ...
            <Card className="shadow-card">
              ...
              <CardContent>
                {vesselIncidents.length === 0 ? (<p className="text-sm text-muted-foreground text-center py-4">
                    Belum ada riwayat kejadian
                  </p>) : (<div className="space-y-3">
                    {vesselIncidents.slice(0, 5).map((incident) => (<div key={incident.id} className="p-3 rounded-lg border border-border hover:border-primary/30 cursor-pointer transition-colors" onClick={() => navigate(`/incidents/${incident.id}`)}>
                        <div className="flex items-center justify-between">
