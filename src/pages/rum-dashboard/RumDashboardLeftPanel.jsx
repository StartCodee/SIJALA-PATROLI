...
import { RUM_FEED_BADGE_CONFIG } from "@/data/rumData";
...
function LeftInfoCell({ title, value }) {
  return (
    <div className="flex min-h-[4.7rem] flex-col items-center justify-center border-r border-white/12 bg-[#1a4452]/55 px-3 py-2 text-center last:border-r-0">
      ...
      <p className="mt-2 text-[11px] font-semibold leading-tight text-white">{value}</p>
    </div>
  );
}
...
function StatusBadge({ label, active, activeClass, inactiveClass }) {
  return (
    <div
      className={`flex min-h-[3.25rem] flex-col justify-center rounded-[7px] px-2 py-1.5 text-center text-[10px] ${
        active ? activeClass : inactiveClass
      }`}
    >
      <span className="font-bold leading-tight text-white">{label}</span>
      <span className="rounded-[5px] bg-black/75 px-1.5 py-1 text-[9px] font-black leading-none text-white">
        {active ? "Ada" : "Tidak"}
      </span>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <div className="mt-2.5 grid grid-cols-3 gap-1.5">
        <StatusBadge
          label={RUM_FEED_BADGE_CONFIG.tetap.label}
          active={item.categoryKey === "tetap"}
          activeClass={RUM_FEED_BADGE_CONFIG.tetap.activeClass}
          inactiveClass={RUM_FEED_BADGE_CONFIG.tetap.inactiveClass}
        />
        <StatusBadge
          label={RUM_FEED_BADGE_CONFIG["tidak-tetap"].label}
          active={item.categoryKey === "tidak-tetap"}
          activeClass={RUM_FEED_BADGE_CONFIG["tidak-tetap"].activeClass}
          inactiveClass={RUM_FEED_BADGE_CONFIG["tidak-tetap"].inactiveClass}
        />
        <StatusBadge
          label={RUM_FEED_BADGE_CONFIG.megafauna.label}
          active={item.categoryKey === "megafauna"}
          activeClass={RUM_FEED_BADGE_CONFIG.megafauna.activeClass}
          inactiveClass={RUM_FEED_BADGE_CONFIG.megafauna.inactiveClass}
        />
      </div>
    </button>
  );
...
export default function RumDashboardLeftPanel({
  selectedKkp,
  selectedPos,
  kkpOptions,
  availablePosOptions,
  ...
  return (
    <aside
      className="flex min-h-0 flex-col space-y-0 overflow-hidden rounded-xl border border-white/15 bg-[#163848]/75 p-0 text-white xl:col-span-3"
    ...
    >
      ...
      <div className="grid grid-cols-3 border-b border-white/12 text-center">
        ...
        />
        <LeftSelectCell
          title="Pos Pengawasan"
          value={selectedPos}
          options={availablePosOptions}
          onChange={onPosChange}
        />
        <LeftInfoCell title="Pemakaian BBM" value={fuelUsageLabel} />
      </div>
      ...
      <div className="grid grid-cols-3">
        ...
        <LeftMiniMetric title="Capaian Luasan Patroli (STD)" value={patrolCoverageLabel} />
      </div>
      ...
      {showEmbedGenerator ? (
        <div className="px-4 pb-4">
          <EmbedCodePanel dashboardPath="/monitoring-megafauna" />
        </div>
      ) : null}
    </aside>
  );
