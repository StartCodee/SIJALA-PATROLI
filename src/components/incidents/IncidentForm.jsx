...
export const IncidentForm = ({ onSuccess, onCancel, submitLabel = 'Simpan Laporan', cancelLabel = 'Batal', showCancel = true, resetOnSuccess = false, className, }) => {
    ...
    return (<form onSubmit={handleSubmit} className={cn('space-y-6', className)}>
      ...
      <div className="space-y-2">
        ...
        <Input id="title" placeholder="Contoh: Dugaan penangkapan ikan ilegal" value={formData.title} onChange={(e) => setFormData(prev => ({ ...prev, title: e.target.value }))}/>
      </div>
      ...
      <div className="space-y-2">
        ...
        <Select value={formData.category} onValueChange={(value) => setFormData(prev => ({ ...prev, category: value }))}>
          ...
          <SelectContent>
            {INCIDENT_CATEGORIES.map((cat) => (<SelectItem key={cat} value={cat}>
                {cat}
              </SelectItem>))}
          </SelectContent>
        </Select>
      ...
      <div className="space-y-2">
        ...
        <Textarea id="description" placeholder="Jelaskan detail kejadian..." rows={4} value={formData.description} onChange={(e) => setFormData(prev => ({ ...prev, description: e.target.value }))}/>
      </div>
      ...
      <div className="space-y-2">
        ...
        <Select value={formData.patrolId} onValueChange={(value) => setFormData(prev => ({ ...prev, patrolId: value }))}>
          ...
          <SelectContent>
            {activePatrols.map((patrol) => (<SelectItem key={patrol.id} value={patrol.id}>
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
              <Input placeholder="Lintang (cth: -0.234)" value={formData.lat} onChange={(e) => setFormData(prev => ({ ...prev, lat: e.target.value }))}/>
          </div>
          <div>
              <Input placeholder="Bujur (cth: 130.512)" value={formData.lon} onChange={(e) => setFormData(prev => ({ ...prev, lon: e.target.value }))}/>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 pt-4">
        ...
        {showCancel && (<Button type="button" variant="outline" onClick={onCancel}>
            {cancelLabel}
          </Button>)}
      </div>
    </form>);
